...
async def konkursi(message: types.Message, state: FSMContext):
    await message.answer("Конкурсы:", reply_markup=Markup_konkurs.get_markup())
...
@router.message(state=AwaitMessages.markup_konkurs)
async def create_markup(message: types.Message, state: FSMContext):
    Database.database_konkurs(message.text)
    await state.clear()
